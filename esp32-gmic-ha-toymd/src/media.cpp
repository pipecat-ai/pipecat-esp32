...
void pipecat_init_audio_capture() {
  // No esp-bsp package exists for a custom board, so bring the codec up
  // directly. Gain and volume are board-electrical properties and are set in
  // board_init().
  board_init();
  mic_codec_dev = board_get_record_handle();
  spk_codec_dev = board_get_playback_handle();
